...
IMPLICIT NONE
   ...
   NNN=0
   WW=0
   CALL PRESSURE_SMOOTH_SHA_K<<<CEILING(REAL(NODEID(-1))/64),64>>>(KW,LNODE,NODN,NODEID,NLMAX,MBA,NWALLID,COORX,COORY,COORZ,R1,PTMP,DDR,P)
   I=cudaDeviceSynchronize()

   DO I=1,NODEID(-1)
      J=NNN(I)
      IF((NWALLID(I,2).EQ.-10).OR.&
      (NWALLID(I,2).EQ.-11).OR.&
      (NODEID(I).EQ.4))THEN    
